...
# remove upgrade flag...
if [ -f /configs/upGradeFlag ]; then
	rm /configs/upGradeFlag
	sync
fi

############# Setting register and insert wifi ko ############
insmod /system/driver/tx-isp-t31.ko clk_name="vpll" isp_clk=212000000
insmod /system/driver/exfat.ko
insmod /system/driver/audio.ko spk_gpio=-1 alc_mode=0 mic_gain=1
...
if [[ "0x024c" == $WifiVendorId ]]; then
	...
	echo 0 > /sys/module/atbm603x_wifi_sdio/parameters/atbm_printk_mask
...
NO_NTP_FLAG_FILE="/tmp/.no_ntp_flag"
touch $NO_NTP_FLAG_FILE
...
if [ ! -f $DEBUG_STATUS ]; then
    ...
    mkdaemon -1 syslogd /sbin/syslogd -C1024 -n -t -S
    ...
    mkdaemon -1 assis /system/bin/assis
    ...
    mkdaemon -1 dnsmasq /system/bin/dnsmasq
    mkdaemon -1 iotclient /system/bin/iotclient
    mkdaemon -1 sinker /system/bin/sinker
    mkdaemon 0 iCamera /system/bin/iCamera
    mkdaemon -1 dumpload /system/bin/dumpload
    mkdaemon -1 timesync /system/bin/timesync
    mkdaemon -1 sinker_checkCloud /system/bin/sinker_checkCloud.sh

	#/system/bin/assis &
	#/system/bin/iotclient &
	#/system/bin/sinker &
	#/system/bin/iCamera &
else
	sleep 0.5
